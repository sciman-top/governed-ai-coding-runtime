...
class AttachedRepoE2ETests(unittest.TestCase):
    def test_runtime_check_executes_attached_write_e2e_with_handoff_and_replay_refs(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertFalse(payload["problem_trace"]["has_problem"])
            self.assertEqual(payload["problem_trace"]["problem_kind"], "none")
            self.assertEqual(payload["problem_trace"]["failure_stage"], "none")
            self.assertEqual(payload["problem_trace"]["target_context"]["task_id"], "task-attached-e2e")
            self.assertEqual(payload["problem_trace"]["target_context"]["run_id"], "run-attached-e2e")
    ...
    def test_runtime_check_preflight_blocks_denied_write_and_emits_retry_action(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertTrue(payload["problem_trace"]["has_problem"])
            self.assertEqual(payload["problem_trace"]["problem_kind"], "write_policy")
            self.assertEqual(payload["problem_trace"]["failure_stage"], "write_preflight")
            self.assertEqual(payload["problem_trace"]["failure_signature"], "write_preflight")
            self.assertTrue(payload["problem_trace"]["write_issue"]["preflight_blocked"])
            self.assertEqual(payload["problem_trace"]["target_context"]["task_id"], "task-attached-preflight")
    ...
    def test_runtime_check_fails_when_target_repo_dependency_baseline_is_missing(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(payload["summary"]["overall_status"], "fail")
            self.assertTrue(payload["problem_trace"]["has_problem"])
            self.assertEqual(payload["problem_trace"]["problem_kind"], "target_repo_contract")
            self.assertEqual(payload["problem_trace"]["failure_stage"], "dependency_baseline")
            self.assertEqual(payload["problem_trace"]["failure_signature"], "dependency_baseline")
            self.assertTrue(payload["problem_trace"]["failure_reason"])
