...
class OperatorUiTests(unittest.TestCase):
    ...
    def test_operator_ui_interactive_mode_renders_actions_and_ref_buttons(self) -> None:
        ...
        self.assertIn("fetch('/api/codex/status?refresh_if_stale=1', { cache: 'no-store' })", html)
        self.assertIn("检测到额度快照过期时，会自动尝试在线刷新。", html)
        ...
        self.assertIn("maybeRefreshCodexStatusOnResume()", html)
        ...
        self.assertIn("createMultilineInfoValue(formatTokenExpirySummary(account))", html)
