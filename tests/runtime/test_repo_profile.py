...
class RepoProfileContractTests(unittest.TestCase):
    ...
    def test_repo_profile_normalizes_workflow_governance_policy(self) -> None:
        module = importlib.import_module("governed_ai_coding_runtime_contracts.repo_profile")
        raw = {
            ...
            "workflow_governance_policy": {
                "default_workflow_mode": "spec_first",
                "allowed_workflow_modes": ["direct_fix", "spec_first", "spec_plus_review"],
                "workflow_mode_overrides_by_risk": {
                    "low": "direct_fix",
                    "medium": "spec_first",
                    "high": "spec_plus_review",
                },
                "spec_artifact_requirements": ["task_brief", "change_plan"],
                "review_requirements": {"medium": "owner_review", "high": "owner_review_required"},
                "worktree_policy": {"allow": False, "fallback_mode": "spec_plus_review"},
                "subagent_policy": {"allow": False, "fallback_mode": "spec_plus_review"},
                "automation_policy": {"allow": True, "fallback_mode": "direct_fix"},
            },
        }

        profile = module.RepoProfile.from_dict(raw)
        self.assertEqual(profile.workflow_governance_policy["default_workflow_mode"], "spec_first")
        self.assertEqual(
            profile.workflow_governance_policy["workflow_mode_overrides_by_risk"]["high"],
            "spec_plus_review",
        )
        self.assertEqual(
            profile.workflow_governance_policy["worktree_policy"]["fallback_mode"],
            "spec_plus_review",
        )

    def test_repo_profile_rejects_invalid_workflow_governance_policy(self) -> None:
        module = importlib.import_module("governed_ai_coding_runtime_contracts.repo_profile")
        raw = {
            "repo_id": "bad-repo",
            "primary_language": "python",
            ...
            "path_policies": {"read_allow": ["src/**"], "write_allow": [], "blocked": []},
            "workflow_governance_policy": {
                "default_workflow_mode": "parallel_subagent_assist",
                "allowed_workflow_modes": ["direct_fix", "spec_first"],
                "workflow_mode_overrides_by_risk": {"low": "direct_fix"},
