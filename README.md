...
`http://127.0.0.1:8770/?lang=zh-CN` 的 Codex 面板也提供“检查共享历史互操作”和“应用共享历史优化”。前者只读检查 `Cockpit Tools` 当前 Codex 账号是否可投影到 Codex App/CLI，并核对 `state_5.sqlite` 中的 `threads.model_provider` 可见性分桶；后者会备份并写入 Codex 配置，把历史 metadata 迁移到稳定内置 `openai` bucket，并 fail-closed 检查/清理非法 `[model_providers.openai]` 覆盖。

与本机 `Cockpit Tools` / `CC Switch` 的衔接边界：`Cockpit Tools` 负责 Codex App/CLI 的 ChatGPT auth 和 Codex API provider 切换；`cc-switch` 负责 Claude CLI、GLM、DeepSeek 等第三方 API 切换。Codex 侧统一读取 Cockpit 当前 Codex account 文件，API key 只注入当前启动进程，并把 RightCode、35.213.82.91 等所有 Cockpit API provider 都归到同一个内置 `model_provider = "openai"` 历史 bucket；中转站地址只写入顶层 `openai_base_url`，不得写 `[model_providers.openai]`。Cockpit 的 `codex_launch_on_switch` 必须保持关闭，避免切号后绕过本仓共享历史修复链直接 raw-launch Codex App。Codex App 对 auth/provider 状态通常按进程启动快照读取；切换 Cockpit 账号后如已打开 App，先跑互操作检查和 restart-guard 备份，确需刷新 UI 时由操作者手动关闭后再用 `codex-cockpit-app` 打开，`codex-cockpit-app-restart` 只作为明确授权后的手动入口。历史 metadata 迁移会先备份 `state_5.sqlite`。Claude 侧连续性由 Claude Code 自身的 session transcript 机制承担，provider 切换只应改 `settings.json` / provider profile / 当前进程 env，不应把每个 provider 切到不同 `CLAUDE_CONFIG_DIR`；若 `cc-switch` 或其他工具启用了独立 context group，需要显式把它视为隔离会话而非共享历史。
