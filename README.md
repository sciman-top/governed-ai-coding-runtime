...
- 核心原则还补强了三条最佳实践边界：`context_budget_and_instruction_minimalism`（上下文预算与指令最小化）、`least_privilege_tool_credential_boundary`（最小权限工具/凭据边界）、`measured_effect_feedback_over_claims`（效果反馈优先于完成声明）。这些原则要求根规则保持短而硬，工具权限/凭据/沙箱可审计，完成声明必须有 fresh target-run evidence、eval trace、effect feedback、verification command 与 rollback。
