...
- Codex 本机配置边界：本仓不再安装或包装 `codex` / Codex App / Cockpit Tools 启动入口，也不再写入 Codex auth、provider、profile、SQLite history bucket 或 Cockpit launch state。旧 `Optimize-CodexLocal.ps1`、`Start-CodexShared.ps1`、`codex-account.py`、`codex-account.ps1` 已删除；只读诊断使用 `python scripts/codex-interop-check.py ... --quick-launch`；清理旧项目 shim 使用 `scripts/Disable-CodexProjectInterop.ps1 -Apply -DisableProjectShortcuts`。
