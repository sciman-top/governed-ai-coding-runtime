...
| Runtime state placement | `scripts/bootstrap-runtime.ps1`, `scripts/run-governed-task.py`, `scripts/serve-operator-ui.py`, and `runtime_status.py` assume `.runtime/` under the current repo root. | Repo-local declarations stay light; mutable runtime state lives in a machine-wide sidecar or equivalent machine-local binding root. | Transitional surface only; conflicts if treated as the final product boundary. | Keep repo-root `.runtime/` as the baseline bootstrap path and introduce machine-local binding state in active productization work. |
...
| Workspace isolation | `workspace.py` enforces isolated relative workspaces under `.governed-workspaces/` and validates write paths from repo profiles. | Workspaces remain isolated and policy-bound, but binding and location become machine-local rather than implicitly repo-root. | Aligned guardrail with location change pending. | Preserve isolation and path validation; decouple workspace and binding roots from the current repo root during attachment work. |
