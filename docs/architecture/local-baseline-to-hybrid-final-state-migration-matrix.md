...
| Codex integration | `trial_entrypoint.py` exposes `codex.cli.compatible` with dynamic capability posture (`native_attach` preferred; fallback to `process_bridge` / `manual_handoff`) while preserving user-owned upstream auth. | Add a direct Codex adapter path while keeping the kernel compatible with future agent products. | Compatible transitional surface with runtime-projected posture. | Keep fallback tiers visible and continue tightening direct adapter evidence mapping in `GAP-037` follow-on hardening. |
...
- fallback-only Codex posture as equivalent to direct attach proof
