...
- 运行时仍保留 direct Codex adapter 的能力探测与会话身份握手面（capability probe + session identity handshake）。
- `scripts/run-governed-task.py` 现在只保留 repo-local `create` / `run` / `status`。
- operator / control-plane 只保留只读查询：status、evidence、handoff、continuity。
- target-repo attachment、session-bridge 和 apply-all 治理流已退役。
...
`scripts/run-governed-task.py` 证明的是本地 governed runtime 路径已经闭环：
- task persistence
...
它应当被理解为 **repo-local runtime task surface**，不是外部目标仓 rollout 机制。

### 3. 只读 Operator Surface
operator 查询面保持只读且只面向本仓：
- `/operator` 暴露 `status`、`inspect_evidence`、`inspect_handoff`
- continuity search/write 只作用于本仓 continuity index
- `python scripts/serve-operator-ui.py` 只是这些只读面的投影，不再恢复 attachment 或写入桥接
...
### 方案 A：Repo-Local 验证与证据闭环（推荐）
适合需要本仓门禁、证据与 operator 可见性，但不需要任何外部目标仓桥接的场景。

1. 先执行：
   - `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts/bootstrap-runtime.ps1`
   - `pwsh -NoProfile -ExecutionPolicy Bypass -File scripts/doctor-runtime.ps1`
2. 运行 repo-local task 或快速门禁：
...
2. 用本仓库做 host capability、repo-local verification、evidence/handoff/continuity 侧的治理留痕。
3. 不要再期待本仓 attach 其他仓库，或代你执行 session-bridge 治理。
...
- 不再拥有 target-repo attachment、light-pack 分发或 session-bridge 执行链
...
- 在 Codex 工作流上可执行的 repo-local 验证与证据层（带 adapter tier 与 degrade 规则）
