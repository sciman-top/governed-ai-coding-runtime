...
- Missing: contract families for hooks, skills, knowledge sources, waivers, provenance, and repo maps; sample repo profiles; sample control packs; compatibility validation; `apps/`, `packages/`, `infra/`, `tests/`; CI; and executable services.

## P0
1. Ratify the governance-kernel boundary and keep roadmap, backlog, and seeding artifacts aligned with it.
2. Add missing governance contract families: hook contract, skill manifest, knowledge source, waiver record, provenance record, and repo-map contract.
3. Add at least two sample repo profiles and one sample control pack.
