...
## 1. 阅读指引（必读）
- 本文件承接 `GlobalUser/CLAUDE.md v9.46`，仅定义本仓落地动作（WHERE/HOW）。
- 固定结构：`1 / A / B / C / D`。
...
- 精简原则：根文件只写生成边界、真实入口、硬门禁、证据与回滚；长命令说明、审查模板和运行样例放入 `docs/`、`.claude/rules/` 或产物证据。
...
- 状态/诊断命令采用“help 探测 -> 有则执行 -> 无则 `platform_na` 落证”；交互场景可用 `/memory` 查加载链。
