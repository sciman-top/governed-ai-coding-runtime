...
**项目**: github-toolkit
**承接来源**: `GlobalUser/GEMINI.md v9.46`
**适用范围**: 项目级（仓库根）
**最后更新**: 2026-04-28
...
- 用 `.geminiignore` 排除临时日志、缓存、本机认证材料和大体积运行态输出；修改后用 `/memory show` 核查完整上下文；来源与刷新命令先看当前 `/memory` help，支持则用 `/memory list` / `/memory refresh`，否则记录版本并用 `/memory reload` 兜底。
