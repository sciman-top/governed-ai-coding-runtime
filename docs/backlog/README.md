...
- `Target Repo Managed Asset Retirement And Uninstall Queue / GAP-144` through `GAP-151` is now machine-visible in the backlog and issue seeds. The current implementation baseline covers identity classification, inventory dry-run, observe-mode managed-file provenance sidecars, retired-file contract, prune, uninstall, and all-target/operator integration; `GAP-151` remains the closeout gate until fresh target-repo dry-run evidence and hard gates are recorded.
