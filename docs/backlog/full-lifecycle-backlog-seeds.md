...
- The lifecycle definition and capability freeze work under `Vision / GAP-018` and `GAP-019` is complete; the active next-step queue begins at `Foundation / GAP-020`.
...
## Foundation
1. Formalize clarification, rollout, compatibility, and evidence maturity.
2. Replace the remaining `build` and `hotspot` placeholders with live commands.
3. Add durable task persistence and workflow skeletons.
4. Add control lifecycle metadata and evidence completeness checks.
5. Treat `docs/plans/foundation-runtime-substrate-implementation-plan.md` as the current execution checklist for this stage.
