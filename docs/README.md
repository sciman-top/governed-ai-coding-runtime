...
- For dual first-class host support, `GAP-115` through `GAP-119` are complete as a bounded governance-result parity queue. The current live posture is intentionally split by host: Claude workload probe is `native_attach` / ready, while Codex target runs remain `process_bridge` / degraded until fresh `ready` plus `native_attach` evidence proves recovery.
...
- `current live posture`: target-run freshness is `fresh`; Codex target runs are still `process_bridge` / degraded; Claude workload probe is `native_attach` / ready
