...
- [Optimized Hybrid Final-State Long-Term Implementation Plan](./optimized-hybrid-final-state-long-term-implementation-plan.md)
  ...
  - Scope: `GAP-093` through `GAP-111`; `GAP-093..103` are complete optimized health/trigger-discipline work, `GAP-104` is complete realization rebaseline, `GAP-105` is complete service-primary runtime boundary work, `GAP-106` is complete live Codex continuity work, `GAP-107` is complete non-Codex parity work, `GAP-108` is complete governed executable tool coverage work, `GAP-109` is complete data/provenance release work, and `GAP-110..111` cover the remaining implementation and certification work required for truthful final-state closure
