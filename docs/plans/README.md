...
- [Codex CLI Continuity And Hot-Switch Plan](./codex-cli-continuity-and-hot-switch-plan.md)
  - Status: owner-directed scoped spike rebaselined and live-smoked on `2026-05-15`, then updated on `2026-05-16` to prefer Cockpit follow-current single-account local access over the former multi-account pool while preserving old direct OAuth/API projection as an explicit compatibility mode
  - Scope: keep Codex CLI segmented continuity as fallback, retain read-only switch-health guardrails, and use `Codex -> LiteLLM -> Cockpit API service` as the preferred opt-in local profile where LiteLLM owns the client-facing gateway while Cockpit owns ChatGPT OAuth account switching, refresh, quota facts, and current-account selection; keep `CodexApiProjectionRepair` and `CodexOauthProjectionRepair` as the old direct Codex App/CLI roundtrip switches
  - Safety boundary: Cockpit Tools remains the ChatGPT OAuth account/quota/switch owner; LiteLLM is the preferred client-facing gateway for normal API keys and multi-provider routing; this repo must not restart Codex App or expose Cockpit's `127.0.0.1:2876/v1` service without listener-scope and firewall evidence.
...
- [Governance Optimization Lane Implementation Plan](./governance-optimization-lane-implementation-plan.md)
  - Status: completed follow-on implementation lane after `GAP-060` closeout (verified on 2026-04-20)
...
3. Use the roadmap and backlog to understand current posture:
   ...
   - use [Optimized Hybrid Final-State Long-Term Roadmap](../roadmap/optimized-hybrid-final-state-long-term-roadmap.md) for the post-review `GAP-093..103` long-term sequencing and `LTP-01..06` trigger gates
...
- Historical plans remain useful evidence. The direct-to-final-state and governance-optimization plans are both complete on the current branch baseline, and completed productization or strategy-alignment plans remain rationale and execution history rather than competing active queues.
