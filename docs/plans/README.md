...
- [Codex CLI Continuity And Hot-Switch Plan](./codex-cli-continuity-and-hot-switch-plan.md)
  - Status: owner-directed scoped spike rebaselined on `2026-05-15`; `CCHS-001..004` remain completed or partial native-boundary groundwork, and `CCHS-005A..005F` are the active gateway lane without live auth/provider mutation by the plan itself
  - Scope: keep Codex CLI segmented continuity as fallback, retain read-only switch-health guardrails, and execute `CCHS-005A` through `CCHS-005F` for the preferred `Codex -> LiteLLM -> Cockpit API service` path where LiteLLM owns normal API-key providers while Cockpit API service can be a hardened opt-in upstream
  - Safety boundary: Cockpit Tools remains the ChatGPT OAuth account/quota/switch owner; LiteLLM is the preferred client-facing gateway for normal API keys and multi-provider routing; this repo must not take over Cockpit config, write auth by default, restart Codex App, or expose Cockpit's `127.0.0.1:2876/v1` service without listener-scope and firewall evidence.
