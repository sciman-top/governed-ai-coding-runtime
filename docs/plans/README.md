...
- [Codex CLI Continuity And Hot-Switch Plan](./codex-cli-continuity-and-hot-switch-plan.md)
  - Status: owner-directed scoped spike rebaselined and live-smoked on `2026-05-15`, then updated on `2026-05-16` to prefer Cockpit follow-current single-account local access over the former multi-account pool
  - Scope: keep Codex CLI segmented continuity as fallback, retain read-only switch-health guardrails, and use `Codex -> LiteLLM -> Cockpit API service` as the preferred opt-in local profile where LiteLLM owns the client-facing gateway while Cockpit owns ChatGPT OAuth account switching, refresh, quota facts, and current-account selection
