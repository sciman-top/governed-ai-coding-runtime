...
- Current state: `GAP-130..143` are complete on the current branch baseline. `GAP-140` established the bounded host-capability defer boundary before recovery was proven, `GAP-141` formalized the historical problem-trace retention and exit rule for rolling KPI windows, `GAP-142` made fresh-but-degraded target-run evidence block implementation work, and `GAP-143` made that recovery posture machine-checkable across selector, host-feedback, and target-repo effect feedback. Fresh 2026-06-09 evidence has now closed the bounded wait: Codex target runs recovered to `ready/native_attach/live_attach`, and the selector returned to `defer_ltp_and_refresh_evidence`. This plan still does not enable automatic policy mutation, automatic skill installation, target-repo sync, push, or merge.
...
Turn fresh but degraded target-run evidence into an `attention` signal and force the autonomous selector to block implementation work; bounded host-capability degradation waits on `wait_for_host_capability_recovery` instead of repeating evidence refresh.
