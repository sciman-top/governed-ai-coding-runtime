# GEMINI.md - Universal Agent Protocol v9.53
...
- 代表用户执行 `git commit` 时，提交信息默认中文优先：`subject` 用简洁中文概括真实改动，必要时保留 `fix:`/`docs:` 等约定前缀和英文标识符；除非用户、仓库规范或自动化模板明确要求英文，否则提交前自检 subject 是否包含中文。
- 编码时适度添加中文注释：只解释不直观的业务规则、边界条件、风险控制、兼容原因或复杂流程；不要给显而易见的赋值/循环写噪声注释；公开 API、协议字段、错误码、第三方接口名和日志关键字保持英文原文。
...
- settings 由默认、system default、user、project、system override、环境变量和 CLI 参数逐层覆盖；诊断时不要只看单个 `settings.json`。
- 启用 Trusted Folders 时，未受信目录会进入受限 safe mode；项目 settings、`.env`、extension、工具自动批准和自动 memory 可能被禁用，必须先核查 trust 状态再判断规则失效。
...
- Gemini CLI 会加载 `.env`/环境变量并可能覆盖配置；涉及凭据、provider、proxy、telemetry 或 sandbox 时必须核查环境来源。
...
## C. 项目级承接契约
### C.1 自包含与边界
- 项目级 `GEMINI.md` 必须显式承接 `GlobalUser/GEMINI.md v9.53`。
