...
TRANSLATIONS = {
    "zh-CN": {
        ...
        "codex_refresh_if_stale": "只刷新本机状态；额度允许延迟。需要即时额度时点“强制在线刷新”。",
        ...
        "codex_recommended_note": "本项目长期优先遵循“综合效率优先”：少打扰、自动连续执行、节省 token / 成本、高效率。`gpt-5.3-codex + medium + never` 只是当前暂行实现；以后若模型、参数或技术栈更迭，应优先保持这个原则，而不是固化当前组合本身。",
        "codex_login": "登录状态",
    ...
    "en": {
        ...
        "codex_refresh_if_stale": "Refreshes local status only; usage may lag. Use Force online refresh for live usage.",
        ...
        "codex_recommended_note": "This project keeps the long-lived principle of efficiency first: low interruption, continuous execution, lower token and cost burn, and high throughput. `gpt-5.3-codex + medium + never` is only the current implementation. When future models, parameters, or tooling change, preserve that principle first rather than freezing the current combo itself.",
