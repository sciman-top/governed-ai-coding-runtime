...
def _render_codex_panel(text: dict[str, str], *, interactive: bool) -> str:
    ...
    return "\n".join(
        [
            ...
            f"<button type='button' class='primary' data-action='codex_interop_repair' data-confirm='{escape(text['codex_interop_repair_confirm'])}'>{escape(text['codex_interop_repair'])}</button>",
            f"<button type='button' data-codex-import-cockpit='1' data-confirm='{escape(text['codex_import_cockpit_confirm'])}'>{escape(text['codex_import_cockpit'])}</button>",
            ...
            f"<button type='button' data-action='codex_guard_start'>{escape(text['codex_guard_start'])}</button>",
            ...
            "</div>",
            "<form id='codex-api-form' class='codex-api-form'>",
            f"<h3>{escape(text['codex_api_profile'])}</h3>",
            "<div class='codex-api-fields'>",
            f"<label>{escape(text['codex_api_name'])}<input type='text' id='codex-api-name' autocomplete='off' placeholder='api-35'></label>",
            f"<label>{escape(text['codex_api_base_url'])}<input type='text' id='codex-api-base-url' autocomplete='off' placeholder='http://35.213.82.91:8003/v1'></label>",
            f"<label>{escape(text['codex_api_key'])}<input type='password' id='codex-api-key' autocomplete='off'></label>",
            "</div>",
            "<div class='codex-api-options'>",
            f"<label class='checkbox-row'><input type='checkbox' id='codex-api-probe' checked><span>{escape(text['codex_api_probe'])}</span></label>",
            f"<label class='checkbox-row'><input type='checkbox' id='codex-api-switch-now' checked><span>{escape(text['codex_api_switch_now'])}</span></label>",
            f"<button type='submit' class='primary' data-confirm='{escape(text['codex_api_save_confirm'])}'>{escape(text['codex_api_save'])}</button>",
            "</div>",
            "</form>",
            "<form id='codex-import-form' class='codex-import-form'>",
            f"<h3>{escape(text['codex_import_payload'])}</h3>",
            "<div class='codex-import-grid'>",
            f"<label>{escape(text['codex_import_format'])}<select id='codex-import-format'><option value='auto'>auto</option><option value='cockpit_tools'>Cockpit Tools</option><option value='cpa'>cpa</option><option value='sub2api'>sub2api</option></select></label>",
            f"<label class='checkbox-row'><input type='checkbox' id='codex-import-probe'><span>{escape(text['codex_import_probe'])}</span></label>",
            f"<label class='checkbox-row'><input type='checkbox' id='codex-import-dry-run'><span>{escape(text['codex_import_dry_run'])}</span></label>",
            "</div>",
            f"<label>{escape(text['codex_import_json'])}<textarea id='codex-import-content' rows='5' placeholder='{escape(text['codex_import_placeholder'])}'></textarea></label>",
            f"<button type='submit' class='primary' data-confirm='{escape(text['codex_import_confirm'])}'>{escape(text['codex_import_save'])}</button>",
            "</form>",
