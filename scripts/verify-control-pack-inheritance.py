...
def _record_na(records: list[dict[str, str]], reason: str, evidence_link: str) -> None:
    records.append(
        {
            "kind": "gate_na",
            "reason": reason,
            "alternative_verification": "Use checked-in control-pack and repo-profile contracts while the missing surface is restored.",
...
def inspect_control_pack_inheritance(
    *,
    ...
    for label, path in required_paths.items():
        if not path.exists():
            _record_na(na_records, f"required file is missing: {label}", str(path))

    if na_records:
        return {"status": "fail", "errors": errors, "na_records": na_records}
    ...
    for entry in matrix.get("unified_governance", []):
            ...
            errors.append(
                {
                    ...
                    "detail": f"repo profile is missing inherited field {profile_field}",
                }
            )
        if not _has_data_path(control_pack, entry["control_pack_reference"]):
            errors.append(
                {
                    "code": "missing_control_pack_reference",
                    "detail": f"control pack is missing inherited reference {entry['control_pack_reference']}",
                }
            )

    repo_profile_paths = _flatten_paths(repo_profile)
    for entry in matrix.get("target_override", []):
        ...
        if _has_data_path(repo_profile, profile_field):
            value: Any = repo_profile
            for part in profile_field.split("."):
                value = value[part]
            if entry["override_rule"] == "must_remain_true" and value is not True:
                errors.append(
                    {
    ...
    for entry in matrix.get("forbidden_override", []):
        for blocked_name in entry.get("blocked_field_names", []):
            if blocked_name in repo_profile or blocked_name in repo_profile_paths:
                errors.append(
                    {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Verify repo-local control-pack inheritance and repo-profile boundaries.")
