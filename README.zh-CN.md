...
- 当前规则协调版本：`rule_release=9.56 / project_contract_version=2.0 / coordination_schema=2.3`。目标范围先从 `D:\CODE` 直接子 Git 根动态发现，再与 9 仓显式 allowlist 对账；发现不会自动纳管，未登记 Git 根或缺失目标会按契约阻断。
