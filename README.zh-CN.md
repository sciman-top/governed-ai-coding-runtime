...
Codex/Cockpit 现在有明确的新旧模式开关。新模式走 `Codex -> LiteLLM -> Cockpit API service`，旧模式保留原来的 Codex App/CLI OAuth/API direct projection 往返能力：
...
`codex-mode-new` 会启用本仓管理的 LiteLLM gateway profile；`codex-mode-old-api` / `codex-mode-old-oauth` 分别调用 `CodexApiProjectionRepair` 和 `CodexOauthProjectionRepair`，继续保留旧的 API/OAuth 直接投影切换能力。以上动作都不得重启 Codex App，也不得恢复后台 guard、generic `--apply`、provider bucket migration、no-op launcher 或 restart wrapper。
